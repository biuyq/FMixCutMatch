...
class MiniImagenet84(Dataset):
    def __init__(self, args, data, labels, train=True, transform=None, target_transform=None, pslab_transform = None, download=False):
        ...
        self.alpha = 0.4
        self._count = 0
        self.z_exp_labels = np.zeros((len(self.train_labels), args.num_classes), dtype=np.float32)
        self.Z_exp_labels = np.zeros((len(self.train_labels), args.num_classes), dtype=np.float32)
        self.z_soft_labels = np.zeros((len(self.train_labels), args.num_classes), dtype=np.float32)
    ...
    def update_labels_randRelab(self, result, train_unlabeled_indexes, rand_ratio):
        self.relabel_indexes = list(train_unlabeled_indexes)

        self.Z_exp_labels[self.relabel_indexes] = self.alpha * self.Z_exp_labels[self.relabel_indexes] + (1. - self.alpha) * result[self.relabel_indexes]
        self.z_exp_labels[self.relabel_indexes] =  self.Z_exp_labels[self.relabel_indexes] * (1. / (1. - self.alpha ** (self._count + 1)))


        #self.soft_labels[relabel_indexes] = result[relabel_indexes]

        if len(self.relabel_indexes)>0:
            self.soft_labels[self.relabel_indexes] = self.z_exp_labels[self.relabel_indexes]
        self._count += 1

        self.train_labels[self.relabel_indexes] = self.soft_labels[self.relabel_indexes].argmax(axis = 1).astype(np.int64)

        print("Samples relabeled with the prediction: ", str(len(self.relabel_indexes)))
