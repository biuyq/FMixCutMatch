...
class Cifar10Train(tv.datasets.CIFAR10):
        ...
        for id in range(self.args.num_classes):
            ...
            for i in range(len(indexes)):
                if i < len(indexes)-clean_per_class:
                    ...
                    self.targets[indexes[i]] = i % self.args.num_classes 
    ...
    def update_labels_randRelab(self, result, train_noisy_indexes, rand_ratio):
        ...
        if self._count >= self.args.epoch_begin:
            ...
            self.relabel_indexes = list(train_noisy_indexes[idx_relab])
            self.soft_labels[self.relabel_indexes] = result[self.relabel_indexes]
            self.targets[self.relabel_indexes] = self.soft_labels[self.relabel_indexes].argmax(axis = 1).astype(np.int64)
    ...
    def __getitem__(self, index):
        ...
        if self.args.DApseudolab == "False":
            ...
            img_pseudolabels = self.transform(img)
