...
from dataset.svhn import get_dataset

sys.path.append('../radam/')
#from radam import RAdam, AdamW

#sys.path.append('../pytorch-auto-augment-master/')
#from auto_augment import AutoAugment

sys.path.append('../utils_fmc/')
...
def parse_args():
    ...
    parser.add_argument('--alpha', type=float, default=0.4, help='Hyper param for loss')
    ...
    parser.add_argument('--network', type=str, default='MT_Net', help='the backbone of the network' )#MT_Net WRN28_2_wn
    parser.add_argument('--seed', type=int, default=123, help='random seed (default: 1)')
    parser.add_argument('--seed_val', type=int, default=456, help='seed for the validation split')
    ...
    parser.add_argument('--use_FMix', type=str, default='True', help='Whether to use FMix')
    ...
    parser.add_argument('--swa_start', type=int, default=0, help='Start SWA')
...
def data_config(args, transform_train, transform_test, dst_folder):
    ...
    ####################################### Train ##########################################################
    trainset, clean_labels, noisy_labels, train_noisy_indexes, train_clean_indexes = get_dataset(args, transform_train, transform_test, dst_folder)
    #print(np.sum(train_clean_indexes)+np.sum(train_noisy_indexes))
